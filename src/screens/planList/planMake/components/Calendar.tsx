    ...
    const generateMatrix = () => {
        ...
        for (let row = 1; row < 7; row++) {
            ...
            for (let col = 0; col < 7; col++) {
                ...
                let displayMonth = isCurrentMonth ? month + 1 : (day <= 0 ? month : month + 2);
                matrix[row][col] = { day: day > 0 && day <= maxDays ? day : null, month: displayMonth, year };
                counter++;
            }
        }
        return matrix;
    ...
    return (
        <View style={styles.container}>
            <View style={styles.header}>
                ...
                {show && (
                    <MonthPicker onChange={onValueChange} value={currentMonth} locale="ko" />
                )}
            </View>
                ...
                <View style={{ flexDirection: 'row' }}>
                    {days.map((day, index) => (
                        <View style={styles.cell} key={index}>
                            <BText fontSize={11} style={{ opacity: 0.3 }}>{day}</BText>
                        </View>
                    ))}
                </View>
                {generateMatrix().slice(1).map((row, rowIndex) => (
                        ...
                        {row.map((item, colIndex) => (
                            <TouchableOpacity
                                style={
                                    styles.cell}
                                key={colIndex}
                                onPress={() => handleDayPress(item.day, item.month, item.year)}
                            >
                                <View style={selectedDates.includes(`${item.year}-${String(item.month).padStart(2, '0')}-${String(item.day).padStart(2, '0')}`) ? styles.rangeDay : null}>
                                    <View style={
                                        selectedStart === `${item.year}-${String(item.month).padStart(2, '0')}-${String(item.day).padStart(2, '0')}` ||
                                            selectedEnd === `${item.year}-${String(item.month).padStart(2, '0')}-${String(item.day).padStart(2, '0')}`
                                            ? styles.selectedDay : null
                                    }>
                                        <MText fontSize={17} color={
                                            selectedStart === `${item.year}-${String(item.month).padStart(2, '0')}-${String(item.day).padStart(2, '0')}` ||
                                                selectedEnd === `${item.year}-${String(item.month).padStart(2, '0')}-${String(item.day).padStart(2, '0')}`
                                                ? fcolor.white
                                                : fcolor.black
