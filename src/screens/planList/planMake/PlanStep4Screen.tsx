...
import { Animated, FlatList, Image, StyleSheet, TextInput, TouchableOpacity, View } from 'react-native';
  ...
  const frd_info = async (name) => {
    try {
      ...
      for (let id = 0; id < frdid.length; id++) {
        ...
        const db = usersCollection.data();
        if (db.nickname == name)
          setFriends(prevState => [...prevState, { nickname: db.nickname, travelType: db.travelType, introduction: db.introduction }]); ``
    ...
    <Animated.View style={{ flex: 1 }}>
      <View style={{ paddingHorizontal: 30 }}>
        ...
        <View style={styles.box}>
          ...
          />
          <TouchableOpacity onPress={() => { frd_info(addfriend) }}>
            <Mt_Icon name='search' size={24} color={fcolor.blue} />
          </TouchableOpacity>
        </View>

        <MText fontSize={15} color={fcolor.gray4} style={{ marginTop: 23 }}>함께하는 친구</MText>
      </View>
      ...
      <View style={{ flex: 1, justifyContent: 'flex-end', marginHorizontal: 30, marginBottom: 64 }}>
        <View style={{ flexDirection: 'row', gap: 21 }}>
          <TouchableOpacity
            style={[
              globalStyles.buttonBase, { flex: 1, backgroundColor: fcolor.lblue3 }
            ]}
            onPress={() => planMContext.setPlanMStep(planMContext.planMStep - 1)}>
            <MText color={fcolor.lblue4}>이전</MText>
          </TouchableOpacity>
          <TouchableOpacity
            style={[
              globalStyles.buttonBase, { flex: 1, backgroundColor: fcolor.blue }
            ]}
            onPress={() => {planMEnd()}}>
