...
const FriendAddScreen = ({ navigation }) => {
  ...
  const copyCode = (text) => {
    try {
      ...
      setModalMessage({ title:'복사 성공', message:'내 친구코드가 복사되었습니다.' });
  ...
  const requestFrd = async (friendCode) => {
    try {
      ...
      setModalMessage({ title:db.nickname, message:'친구 요청을 보냈어요!\n수락할 때까지 조금만 기다려주세요 😊' });
    } catch {
      setModalMessage({ title:'친구 추가를 완료하지 못했어요', message:'이 친구코드는 없는 친구코드에요.\n다시 확인해주세요!' });
    }
    setModalVisible(true);
  }
