...
const ProfileEditScreen = ({navigation}) => {
  ...
  const validationNext = () => {
    return (
      validateNickname(form.nickname) && validateIntroduction(form.introduction)
    );
  };
  ...
  const showModal = state => {
    if (state) {
      setModalMessage({
        title: '프로필 수정을 완료했어요',
        message: '프로필이 성공적으로 업데이트되었어요!',
      });
    } else {
      setModalMessage({
        title: '프로필 수정을 완료하지 못했어요',
        message:
          '이 닉네임은 이미 누군가 사용 중이에요.\n다른 닉네임을 시도해 보세요!',
      });
    }
    ...
    <View style={[globalStyles.container, {paddingHorizontal: 30}]}>
      ...
      <View style={globalStyles.centered}>
        ...
        <View
          ...
          {form.nickname && (
            <MText fontSize={11} color={fcolor.orange}>
              특수 문자 제외, 최소 2 ~ 10자까지 작성 가능합니다
            </MText>
          )}
        </View>
        <View style={styles.box}>
          <BText fontSize={13} color={fcolor.gray4} style={{marginRight: 24}}>
            한 줄 소개
          </BText>
          <TextInput
            style={{fontSize: 13}}
        ...
        <TouchableOpacity
          style={[
            ...
            validationNext() ? {backgroundColor: fcolor.blue} : null,
          ]}
